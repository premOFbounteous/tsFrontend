...
// const HeroSection = () => (
//   <div className="bg-yellow-50 rounded-3xl my-8 p-8 md:p-12">
//     <div className="flex flex-col md:flex-row items-center gap-8">
//       <div className="flex-1 text-center md:text-left">
//         <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight text-gray-900 leading-tight">
//           LET'S
//           <br />EXPLORE
//           <br /><span className="bg-yellow-400 px-4 rounded-md">UNIQUE</span>
//           <br />CLOTHES.
//         </h1>
//         <p className="mt-6 text-lg text-gray-600">Live for influential and innovative fashion!</p>
//         <a href="#products-section" className="inline-block mt-8 bg-black text-white px-8 py-4 rounded-lg font-semibold text-lg hover:bg-gray-800 transition-transform hover:scale-105 shadow-lg">Shop Now</a>
//       </div>
//       <div className="flex-1 mt-8 md:mt-0">
//           <img src="https://i.imgur.com/qdi1N5k.png" alt="Fashion model in a pink fluffy coat" className="w-full h-auto rounded-2xl object-cover" 
//           onError={(e:any) => e.target.src='https://placehold.co/600x600/fefce8/111827?text=Style'}/>
//       </div>
//     </div>
//   </div>
// );
...
//                     <input type="email" placeholder="Add your email here" className="flex-1 rounded-l-lg px-4 py-3 outline-none"/>
...
import React, { useEffect, useRef, useState } from "react";
import { Search, Mic, X, User, ShoppingCart, Menu, Minus, Plus, Star, Heart, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function App() {
  ...
  const handleCategorySelect = (category: string) => {
    ...
    setPage(1);
    // Smooth scroll to the products section for better UX
...
const slides = [
  {
    ...
    img: "https://images.pexels.com/photos/1926769/pexels-photo-1926769.jpeg?auto=compress&cs=tinysrgb",
  ...
  },
  {
    ...
    description: "Sale! Up to 50% off!",
    img: "https://images.pexels.com/photos/1021693/pexels-photo-1021693.jpeg?auto=compress&cs=tinysrgb",
...
const Slider = () => {
  ...
  useEffect(() => {
    ...
    }, 4000); // Changed to 4 seconds for a better viewing experience

    return () => clearInterval(interval);
  }, []);

  return (
    <div className="h-[calc(100vh-80px)] overflow-hidden relative my-8 rounded-3xl">
      <div
        className="w-max h-full flex transition-all ease-in-out duration-1000"
        style={{ transform: `translateX(-${current * 100}%)` }} // Changed to % to work with relative parent
      >
        {slides.map((slide) => (
          <div
            className={`${slide.bg} w-[calc(100vw-2rem)] max-w-7xl h-full flex flex-col gap-8 md:flex-row`}
            key={slide.id}
          >
            {/* TEXT CONTAINER */}
            <div className="h-1/2 md:w-1/2 md:h-full flex flex-col items-center justify-center gap-8 text-center p-4">
  ...
  onCategorySelect: (category: string) => void;
    ...
    return (
        <div className="py-8 relative">
            ...
            <button 
                onClick={() => scroll('left')}
                className={`absolute left-0 top-1/2 -translate-y-1/2 z-10 p-2 bg-white/80 rounded-full shadow-md hover:bg-white transition-all duration-300 ${isAtStart ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
            ...
            <div 
                ref={scrollRef} 
                className="flex items-center gap-4 overflow-x-auto pb-4 scroll-smooth [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
            >
                {categories.map((category) => (
                    ...
                    >
                        <div className="aspect-square w-full bg-gray-100 rounded-2xl overflow-hidden transition-all duration-300 group-hover:shadow-xl group-hover:-translate-y-1">
                            <img 
                                src={`/pics/${category}.jpg`} 
                                alt={category} 
                                className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110"
                                onError={(e:any) => e.target.src=`https://placehold.co/300x300/e2e8f0/334155?text=${category}`}
                            />
                        </div>
                        <p className="mt-2 font-semibold text-gray-700 capitalize group-hover:text-black">{category.replace(/-/g, ' ')}</p>
                    </div>
                ))}
            </div>
            ...
            <button 
                ...
                className={`absolute right-0 top-1/2 -translate-y-1/2 z-10 p-2 bg-white/80 rounded-full shadow-md hover:bg-white transition-all duration-300 ${isAtEnd ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
